...
module.exports = (bot, utils, ytdl, config) => {
    ...
    fs.readdir("./commands/", (err, files) => {
        ...
        if (jsfiles.length <= 0) return console.log("Başlatma komutu yok...");
    ...
    bot.loadCommand = (commandName) => {
        try {
            ...
            return utils.cmd_fail(`Hata: ${err}\nCommand \`${commandName}\` bulunamıyor.`, `${config.prefix}reload <command>`);
        }
    };
    ...
    bot.handleVideo = async (video, message, vc, playlist = false) => {
        ...
        if (!queue) {
            ...
            try {
                ...
                console.error(`Bulunduğun ses kanalına giremiyorum : ${err}`);
            }
        } else {
    ...
    bot.play = (guild, music) => {
        ...
        queue.textChannel.send(` **${music.title}** Şimdi oynatılıyor!`);
    }
